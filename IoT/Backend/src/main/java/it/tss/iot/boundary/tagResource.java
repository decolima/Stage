...
import it.tss.iot.control.tagStore;
...
public TAG create(TAG biglietto) {
    try {
        ...
        TAG.Tipo tipo = validateTipo(biglietto.getTipo());

        // Impostare le proprietà selezionate nell'oggetto TAG
        biglietto.setProgrammazione(programmazione);
        biglietto.setUtente(utente);
        biglietto.setTipo(tipo);

        // Salva l'oggetto TAG
        TAG savedBiglietto = store.save(biglietto);

        // Restituisci l'oggetto TAG salvato
...
private TAG.Tipo validateTipo(TAG.Tipo tipo) {
    if (tipo == null) {
    ...
    public TAG find(@PathParam("id") Long id) {
        TAG found = store.findById(id).orElseThrow(() -> new NotFoundException());
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<TAG> byProgrammazioneId(@PathParam("id") Long programmazione_id) {
