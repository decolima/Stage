...
interface ZXingReaderModule extends EmscriptenModule {
    readBarcodeFromImage(bufferPtr: number, bufferLength: number, tryHarder: boolean, formats: string, maxNumberOfSymbols: number): ZXingVector<ZXingReadInnerOutput>;
    readBarcodeFromPixmap(bufferPtr: number, imgWidth: number, imgHeight: number, tryHarder: boolean, formats: string, maxNumberOfSymbols: number): ZXingVector<ZXingReadInnerOutput>;
}
...
export declare function readBarcodeFromImageFile<T extends "reader">(imageFile: Blob | File, { tryHarder, formats, maxNumberOfSymbols, }: ZXingReadOptions | undefined, zxingModuleFactory: ZXingModuleFactory<T>): Promise<ZXingReadOutput[]>;
export declare function readBarcodeFromImageData<T extends "reader">(imageData: ImageData, { tryHarder, formats, maxNumberOfSymbols, }: ZXingReadOptions | undefined, zxingModuleFactory: ZXingModuleFactory<T>): Promise<ZXingReadOutput[]>;
